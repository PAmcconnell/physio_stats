# Script name: visualize_physio_ecg.py
...
def plot_all_ecg(ecg_files_derivatives):
    ...
    for file in ecg_files_derivatives:
        try:
            ...
            if 'ecg' in ecg_data.columns:
                # Assuming each file represents a run, append the ecg data
                all_ecg_data.append(ecg_data['ecg'])
            else:
                logging.warning(f"Column 'ecg' not found in file: {file}")
    ...
    if all_ecg_data:
        ...
        plt.fill_between(range(len(mean_ecg)), mean_ecg - mad_ecg, mean_ecg + mad_ecg, color='gray', alpha=0.25, label='Median Absolute Deviation')
...
def plot_group_ecg(all_ecg_data_df, group_name, color, ax):
    ...
    ax.plot(range(len(mean_ecg)), mean_ecg, label=f'{group_name} Mean', color=color, linewidth=1.5, alpha=0.7,zorder=2)
...
def main():
        ...
        for trial_index, trial_type in enumerate(['sequence', 'random']):
            ...
            for group_name, group_df in participants_df.groupby('group'):
                ...
                for participant_id in participant_ids:
                    ecg_files = [os.path.join(derivatives_dir, f) for f in os.listdir(derivatives_dir) if f'{participant_id}_ses-2_task-learn_run-{run_number:02d}_recording-{trial_type}_physio.tsv.gz' in f]
                    logging.info(f"Found {len(ecg_files)} files for participant {participant_id}")
                ...
                if run_data:
                    ...
                    plot_group_ecg(run_data_df, group_name, group_colors[group_name], ax)
   ...
   # Add a single legend for all groups at the bottom of the figure
    ...
    fig.tight_layout()
    plt.savefig(f"{derivatives_dir}/plots/all_subs_ses-2_task-learn_all_events_ecg.png")
    #plt.show()
