# Script name: visualize_physio_eda.py
...
# Plot eda data across all participants.
def plot_all_eda(eda_files_derivatives):
    """
    Plot eda data for all subjects and runs with a Median Absolute Deviation (MAD) band.

    This function reads eda data from provided file paths, calculates the mean and MAD,
    ...
    Parameters:
    - eda_files_derivatives: List of file paths to the eda derivative data files.
    """
    logging.info("Plotting eda data for all subjects and runs.")

    all_eda_data = []  # List to hold all eda data across subjects and runs

    # Aggregate eda data from all files
    for file in eda_files_derivatives:
        try:
            with gzip.open(file, 'rt') as f:
                eda_data = pd.read_csv(f, delimiter='\t')
            if 'eda' in eda_data.columns:
                # Assuming each file represents a run, append the eda data
                all_eda_data.append(eda_data['eda'])
            else:
                logging.warning(f"Column 'eda' not found in file: {file}")
        except Exception as e:
            logging.error(f"Error reading file {file}: {e}")

    # Process and plot data if available
    if all_eda_data:
        # Concatenate all eda data into a single DataFrame
        all_eda_data_df = pd.concat(all_eda_data, axis=1)
        # Calculate the mean and MAD
        mean_eda = all_eda_data_df.mean(axis=1)
        mad_eda = robust.mad(all_eda_data_df, axis=1)

        # Plotting
        plt.figure(figsize=(10, 6))
        plt.plot(mean_eda, label='All Subjects Mean eda')
        plt.fill_between(range(len(mean_eda)), mean_eda - mad_eda, mean_eda + mad_eda, color='gray', alpha=0.5, label='Median Absolute Deviation')
        plt.title('Mean eda with Median Absolute Deviation')
        plt.xlabel('Samples')
        plt.ylabel('Amplitude (microsiemens)')
        plt.legend()
        plt.show()  # Display the plot.
    else:
        logging.warning("No eda data found to plot.")

# Plot eda data seperately for stroke and control groups.
def plot_group_eda(all_eda_data_df, group_name, color, ax):
    """
    Plot eda data for a specific group with a Median Absolute Deviation (MAD) band.

    This function takes pre-processed and concatenated eda data, calculates the mean and MAD,
    and plots these on the provided axes object. The mean is plotted as a line, and the MAD
    is visualized as a shaded band around the mean.

    Parameters:
    - all_eda_data_df: DataFrame containing the concatenated eda data.
    - group_name: Name of the group (e.g., 'stroke', 'control') for labeling.
    - color: Color to use for the mean eda plot of the group.
    - ax: Matplotlib axes object for plotting.
    """
    logging.info(f"Plotting eda data for group: {group_name}")
    ...
    ax.fill_between(range(len(mean_eda)), mean_eda - mad_eda, mean_eda + mad_eda, color=color, alpha=0.3, label=f'{group_name} MAD', zorder=1)
   
    # Plot the mean eda line on top
    ax.plot(range(len(mean_eda)), mean_eda, label=f'{group_name} Mean', color=color, linewidth=2, zorder=2)

# Main script logic
def main():
    """
    Main function to visualize eda data from a BIDS dataset.

    The function performs the following steps:
    - Validates the presence of necessary directories and files.
    - Reads the participants.tsv file to determine the groups.
    - Plots the eda data for each group, including the mean and standard deviation bands.
    """
    ...
    for run_number in range(1, 5):  # Process each run
        ...
        for group_name, group_df in groups:
            ...
            # Collect eda data for all participants in the group for the current run
            run_data = []
            session_id = "ses-1"
            for participant_id in participant_ids:
                # Retrieve eda files for the current run and participant
                eda_files = [os.path.join(derivatives_dir, f) for f in os.listdir(derivatives_dir) if f'{participant_id}_{session_id}_task-rest_run-{run_number:02d}_physio.tsv.gz' in f]
                logging.info(f"Filepaths: {eda_files}")
                logging.info(f"Found {len(eda_files)} eda files for participant {participant_id} and run {run_number}")
                for file in eda_files:
                    try:
                        ...
                        if 'eda' in eda_data.columns:
                            ...
                            # Apply smoothing to the eda data
                            smoothed_eda = smooth_data(eda_data['eda'])
                            run_data.append(smoothed_eda)
                        else:
                            logging.warning(f"Column 'eda' not found in file: {file}")
                    except Exception as e:
                        logging.error(f"Error reading file {file}: {e}")
            ...
            if run_data:
                ...
                plot_group_eda(run_data_df, group_name, color, axs[run_number - 1])
    ...
    for i, ax in enumerate(axs):
        ...
        ax.set_ylabel('Amplitude (microsiemens)')
