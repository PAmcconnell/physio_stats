                ...
                if os.path.isfile(tsv_path):
                    ...
                    if peaks_col not in df.columns:
                        ...
                        if len(peaks_indices) > 1:
                            ...
                            # Plot SD2 arrow extending from bottom left to top right
                            fig.add_shape(type="line", x0=x_min, y0=y_min,
                                          x1=x_max, y1=y_max,
                                          line=dict(color=complementary_color, width=2),
                                          row=row, col=col)

                            # Calculate the mean heart period
                            mean_heart_period = np.mean(rri)

                            # Calculate the coordinates for the SD2 arrow (45 degrees)
                            sd2_angle = np.arctan(1)  # 45 degrees (arctan(1) gives 45 degrees)
                            sd2_arrow_x = mean_heart_period + sd2 * np.cos(sd2_angle)
                            sd2_arrow_y = mean_heart_period + sd2 * np.sin(sd2_angle)
                            fig.add_shape(type="line", x0=mean_heart_period, y0=mean_heart_period,
                                        x1=sd2_arrow_x, y1=sd2_arrow_y,
                                        line=dict(color=complementary_color, width=2),
                                        row=row, col=col)
                            logging.info(f"SD2 arrow: ({mean_heart_period}, {mean_heart_period}) to ({sd2_arrow_x}, {sd2_arrow_y})")

                            # Calculate the coordinates for the SD1 arrow (perpendicular to SD2)
                            sd1_angle = sd2_angle + np.pi / 2  # 90 degrees from SD2
                            sd1_arrow_x = mean_heart_period + sd1 * np.cos(sd1_angle)
                            sd1_arrow_y = mean_heart_period + sd1 * np.sin(sd1_angle)
                            fig.add_shape(type="line", x0=mean_heart_period, y0=mean_heart_period,
                                        x1=sd1_arrow_x, y1=sd1_arrow_y,
                                        line=dict(color=run_color, width=2),
                                        row=row, col=col)
                            logging.info(f"SD1 arrow: ({mean_heart_period}, {mean_heart_period}) to ({sd1_arrow_x}, {sd1_arrow_y})")
