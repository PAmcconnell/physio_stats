            ...
            if os.path.isfile(tsv_path):
                ...
                if peaks_col not in df.columns:
                    ...
                    if len(peaks_indices) > 1:
                        ...
                        logging.info(f"ax1 head: {ax1[:5]}, tail: {ax1[-5:]}")
                        logging.info(f"ax2 head: {ax2[:5]}, tail: {ax2[-5:]}")
                        ...
                        fig.update_xaxes(title_text="RR<sub>n</sub> (ms)", range=[-4000, 4000], row=row, col=col)
                        fig.update_yaxes(title_text="RR<sub>n+1</sub> (ms)", range=[-4000, 4000], row=row, col=col)
                        logging.info(f"Added Poincare plot for method {method_name} to subplot {row}, {col}")

                        # Add SD1 and SD2 annotations
                        fig.add_annotation(x=-3500, y=3500, text=f'SD1: {sd1:.2f}', showarrow=False, row=row, col=col)
                        fig.add_annotation(x=-3500, y=3250, text=f'SD2: {sd2:.2f}', showarrow=False, row=row, col=col)
                    else:
