...
def plot_group_ppg(all_ppg_data_df, group_name, color, ax):
    ...
    ax.plot(range(len(mean_ppg)), mean_ppg, label=f'{group_name} Mean', color=color, linewidth=1.5, alpha=0.7,zorder=2)
...
def main():
        ...
        for trial_index, trial_type in enumerate(['sequence', 'random']):
            ...
            for group_name, group_df in participants_df.groupby('group'):
                ...
                for participant_id in participant_ids:
                    ppg_files = [os.path.join(derivatives_dir, f) for f in os.listdir(derivatives_dir) if f'{participant_id}_ses-2_task-learn_run-{run_number:02d}_recording-{trial_type}_physio.tsv.gz' in f]
                    logging.info(f"Found {len(ppg_files)} files for participant {participant_id}")
                ...
                if run_data:
                    ...
                    plot_group_ppg(run_data_df, group_name, group_colors[group_name], ax)
   ...
   # Add a single legend for all groups at the bottom of the figure
    ...
    fig.tight_layout()
    plt.savefig(f"{derivatives_dir}/plots/all_subs_ses-2_task-learn_all_events_ppg.png")
    #plt.show()
