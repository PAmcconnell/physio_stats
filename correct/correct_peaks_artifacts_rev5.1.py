        ...
        if artifact_windows:
            ...
            if 'start' in latest_artifact and 'end' in latest_artifact:
                ...
                if start < end:
                    ...
                    # Set acceptable threshold for individual deviations in ms
                    deviation_threshold = 25
                    
                    # Check if any individual deviations exceed the threshold of 25 ms
                    deviations_exceed_threshold = np.any(rr_deviations > deviation_threshold)
                    logging.info(f"Do any R-R interval deviations exceed the threshold? {deviations_exceed_threshold}")
                    
                    # Calculate the excess deviations beyond the threshold
                    excess_deviations = rr_deviations - deviation_threshold
                    logging.info(f"Excess deviations beyond the threshold: {excess_deviations}")

                    # Apply stretching or squeezing if any deviations exceed the threshold
                    if deviations_exceed_threshold:
                        logging.info("Adjustments required for R-R intervals exceeding the threshold.")

                        # Determine stretch or compression factor for each R-R interval that exceeds the threshold
                        individual_factors = np.ones_like(concatenated_rr_intervals)
                        individual_factors[deviations_exceed_threshold] = (
                            1 - excess_deviations[deviations_exceed_threshold] / concatenated_rr_intervals[deviations_exceed_threshold]
                        )
                        logging.info(f"Individual stretch/compression factors: {individual_factors}")

                        for i, (start, end) in enumerate(zip(peaks_indices, peaks_indices[1:])):
                            if i < len(individual_factors) - 1:  # exclude the last factor, as it applies to the segment after the last peak
                                stretch_factor = individual_factors[i]
                                logging.info(f"Applying stretch factor to beat {i}: {stretch_factor}")

                                # Calculate the number of samples between the current and next peak
                                segment_length = end - start

                                # Interpolate the average beat to fit the segment length
                                x_old = np.linspace(0, 1, len(average_beat_array))
                                x_new = np.linspace(0, 1, segment_length)
                                adjusted_segment = np.interp(x_new, x_old, average_beat_array * stretch_factor)

                                # Replace the corresponding segment in valid_ppg with the adjusted beat
                                valid_ppg[start:end] = adjusted_segment
                                logging.info(f"Replaced segment {start}:{end} in valid_ppg.")

                        # Recalculate R-R intervals after adjustment
                        new_rr_intervals = np.diff(peaks_indices) / sampling_rate * 1000
                        new_rr_deviations = np.abs(new_rr_intervals - local_rr_interval)
                        logging.info(f"New R-R intervals after individual adjustments: {new_rr_intervals}")
                        logging.info(f"New individual R-R interval deviations: {new_rr_deviations}")

                        # Check if new deviations are within the acceptable range
                        if np.all(new_rr_deviations <= deviation_threshold):
                            logging.info("All R-R interval deviations are now within the acceptable range.")
                        else:
                            logging.error("Some R-R interval deviations still exceed the acceptable range after individual adjustments.")
                    else:
                        logging.info("No individual R-R interval deviations exceed the threshold. No adjustments required.")
